...
// faster approach
//    memset(dest->data, value, sizeof(fix16_t) * dest->rows * dest->columns);
}
...
/*******************************************************
 * General matrix inverse by Gauss-Jordian elimination *
 *******************************************************/

void mf16_invert_gj(mf16 *dest, mf16 *matrix)
{

  // Algorithm for Gauss-Jordan elimination
  // were taken from https://ru.wikipedia.org/wiki/%D0%9C%D0%B5%D1%82%D0%BE%D0%B4_%D0%93%D0%B0%D1%83%D1%81%D1%81%D0%B0_%E2%80%94_%D0%96%D0%BE%D1%80%D0%B4%D0%B0%D0%BD%D0%B0
  mf16 tmp;
  fa16_unalias2(dest, (void **) &matrix, &tmp, sizeof(tmp));
  ...
  // initalize dest matrix and fill it as identity matrix
  dest->rows = matrix->rows;
  dest->columns = matrix->columns;
  mf16_fill(dest, F16(0.0));
  mf16_fill_diagonal(dest, fix16_one);

  // initial pivoting
  for(k = 0; k < dest->rows; k++)
  {
    if (fix16_abs(matrix->data[k][0]) > max)
    {
      max = fix16_abs(matrix->data[k][0]);
      maxrow = k;
    }
  }
  mf16_swaprows(matrix, 0, maxrow);
  mf16_swaprows(dest, 0, maxrow);

  // Direct step
  for(k = 0; k < dest->rows; k++)
  {
    for(i = k + 1; i < dest->columns; i++)
    {
      for(j = 0; j < dest->rows; j++)
      {
        dest->data[i][k] = ratio = fix16_div(dest->data[i][k], matrix->data[i][i]);
        dest->data[i][j] -= fix16_mul(ratio, matrix->data[i][k]);
      }
    }
  }

  //Backward step
  for(k = dest->columns - 1; k ; k--)
  {
    for(i = 0; i < k - 1; i++)
    {
      for(j = 0; j < dest->rows; j++)
      {
        dest->data[i][k] = ratio = fix16_div(dest->data[j][i], matrix->data[i][i]);
        dest->data[i][j] -= fix16_mul(ratio, matrix->data[i][k]);
      }
    }
  }
...
void mf16_trace(fix16_t* trace, mf16* matrix)
{
  uint_fast8_t i;
...
/*******************************************************
 *       Matrix diagonal produclion calculation        *
 *******************************************************/

void mf16_diag_product(fix16_t* permanent, mf16* matrix)
{
  if (matrix->columns != matrix->rows)
  {
    return;
  ...
  *permanent = F16(1.0);
  ...
  {
    *permanent *= matrix->data[i][i];
  }
}

void mf16_lup_determinant(fix16_t* determinant, mf16* matrix, uint8_t* p)
{
  ...
  fix16_t permanent = F16(1.0);

  mf16_diag_product(&permanent, matrix);

  if (p[matrix->rows] % 2 == 0)
  {
    permanent *= -F16(1.0);
  }
...
void mf16_lup_decompose(mf16* matrix, uint8_t* p)
{

  if (matrix->rows != matrix->columns)
  {
    matrix->errors |= FIXMATRIX_DIMERR;
    return;
  }

  uint_fast8_t i, j, k, row;
  ...
  for (i = 0; i < matrix->rows; i++)
  {
    fix16_t pivotValue = 0;
    uint_fast8_t pivotRow = i;
    for (row = i; row < matrix->rows; row++)
    {
      if (fix16_abs(matrix->data[row][i]) > pivotValue)
      {
        pivotValue = fix16_abs(matrix->data[row][i]);
        pivotRow = row;
      }
    }
    ...
    if (pivotValue)
    {
      ...
      if ( pivotRow != i)
      {
        mf16_swaprows(matrix, pivotRow, i);
        p[i] = pivotRow;
        p[matrix->rows]++;
      }
      ...
      {
        matrix->data[j][i] = fix16_div(matrix->data[j][i], matrix->data[i][i]);
        for( k = i + 1; k < matrix->columns; k++ )
        {
          matrix->data[j][k] = fix16_sub(matrix->data[j][k],
                                         fix16_mul(matrix->data[j][i],
                                                   matrix->data[i][k]));
        }
      }
...
void mf16_lup_solve(fix16_t* x, mf16* matrix, fix16_t* b, uint8_t* p)
{
  ...
  for (i = 0; i < matrix->rows; i++) {
    ...
    for (k = 0; k < i; k++)
      x[i] -= matrix->data[i][k] * x[k];
  }

  for (i = matrix->rows - 1; i; i--) {
    for (k = i + 1; k < matrix->columns; k++)
      x[i] = fix16_sub(x[i], fix16_mul(matrix->data[i][k], x[k]));

    x[i] = fix16_div(x[i], matrix->data[i][i]);
  }
}

static inline void mf16_swaprows(mf16* matrix, uint_fast8_t first, uint_fast8_t second)
{
  fix16_t temp[FIXMATRIX_MAX_SIZE];

  memcpy(&temp, &(matrix->data[first][0]), sizeof(fix16_t) * FIXMATRIX_MAX_SIZE);
  memcpy(&(matrix->data[first][0]), &(matrix->data[second][0]), sizeof(fix16_t) * FIXMATRIX_MAX_SIZE);
  memcpy(&(matrix->data[second][0]), &temp, sizeof(fix16_t) * FIXMATRIX_MAX_SIZE);
